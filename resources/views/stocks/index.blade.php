...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">Stocks</h4>
...
@section('content')
    <div class="col-sm-6 col-md-4 col-xl-3">
        <a class="modal-effect btn btn-outline-primary btn-block" data-effect="effect-scale" data-toggle="modal"
            href="#modaldemo8">
            <i class="mdi mdi-account-plus"></i> Add a product
        </a>
    </div>

    <form action="{{ route('stocks.store') }}" method="PUT" class="modal" id="modaldemo8">
        @method('POST')
        @csrf
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content modal-content-demo">
                ...
                <div class="modal-body">
                    <div class="card  box-shadow-0">
                        <div class="card-header">
                            <p class="mb-2">Please enter product data</p>
                        </div>
                        <div class="card-body pt-0">
                            <div class="form-group">
                                <input type="text" class="form-control" required placeholder="product name"
                                    name="product_name">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <select name="trader_id" id="trader_id" class="form-control">
                                    @foreach ($traders as $trader)
                                        <option value="{{ $trader->id }}" {{ $trader->name ? 'selected' : '' }}>
                                            {{ $trader->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <label for="stock_status_id">product status</label>
                                {{-- status --}}
                                <select name="stock_status_id" id="stock_status_id" class="form-control">
                                    @foreach ($stockstatuses as $stockstatus)
                                        <option value="{{ $stockstatus->id }}"
                                            {{ $stockstatus->status ? 'selected' : '' }}>
                                            {{ $stockstatus->status }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                ...
                <div class="modal-footer">
                    <input class="btn ripple btn-primary" type="submit" value="Add Product">
    ...
    <table class="table text-md-nowrap" id="example1">
        ...
        <tbody>
            @foreach ($stocks as $stock)
                <tr>
                    <td style="text-align: center; vertical-align: middle; font-size: 16px;">{{ $stock->id }}</td>
                    <td style="text-align: center; vertical-align: middle; font-size: 16px;">{{ $stock->product_name }}
                    </td>
                    <td style="text-align: center; vertical-align: middle; font-size: 16px;">{{ $stock->description }}
                    </td>
                    <td style="text-align: center; vertical-align: middle; font-size: 16px;">{{ $stock->quantity }}</td>
                    <td style="text-align: center; vertical-align: middle; font-size: 16px;">{{ $stock->price }}</td>
                    <td style="text-align: center; vertical-align: middle; font-size: 16px;">{{ $stock->trader->name }}
                    </td>
                    <td style="text-align: center; vertical-align: middle; font-size: 16px;">
                        {{ $stock->stockstatus->status }}</td>


                    <td style="display: flex; justify-content: center; align-items: center; gap: 10px;">
                        <button class="btn btn-outline-dark" data-toggle="modal"
                            data-target="#editModal{{ $stock->id }}" style="font-size: 14px;">
                            <i class="mdi mdi-account-edit"></i> Edit Stock
                        </button>
                        <form action="{{ route('stocks.destroy', $stock->id) }}" method="post">
                            @method('DELETE')
                            @csrf
                            <button class="btn btn-outline-danger" style="font-size: 14px;">
                                <i class="mdi mdi-account-minus"></i> Delete Stock
                            </button>
                        </form>
                ...
                <!-- Edit Modal for trader {{ $stock->id }} -->
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form class="form-horizontal" method="POST"
                                    ...
                                    <div class="form-group">
                                        <input type="text" class="form-control" required placeholder="Product name"
                                            name="product_name" value="{{ $stock->product_name }}">
                                    </div>
                                    <div class="form-group">
                                        <input type="text" class="form-control" required placeholder="description"
